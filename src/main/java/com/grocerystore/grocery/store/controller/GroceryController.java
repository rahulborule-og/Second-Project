...
@PutMapping("{id}")
    public ResponseEntity<GroceryDao> updateGrocery(@PathVariable("id") Long id,@RequestBody GroceryDao gdao){
    GroceryDao updatedgro=groser.updateDetails(id,gdao);
    return ResponseEntity.ok(updatedgro);
}
}
